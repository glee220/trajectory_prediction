...
###########################################  map_encoder    ############################################################
########################################################################################################################
class MultiScaleAttnMapEncoder(nn.Module):
    def __init__(self, map_channels=3, llm_dim=4096):
        super().__init__()
        
        # 多尺度特征提取
        self.conv1 = nn.Sequential(
            nn.Conv2d(map_channels, 64, 5, stride=1, padding=2),
            nn.ReLU(),
            nn.MaxPool2d(2))  # 1/2分辨率
        
        self.conv2 = nn.Sequential(
            nn.Conv2d(64, 128, 3, stride=1, padding=1),
            nn.ReLU(),
            nn.MaxPool2d(2))  # 1/4分辨率
    ...
    def forward(self, x):
        # 多尺度处理
        x1 = self.conv1(x)  # 中等分辨率特征
        x2 = self.conv2(x1) # 低分辨率特征
        
        # 注意力加权
        attn_map = self.attn(x2)
        weighted_feat = x2 * attn_map
        
        return self.proj(weighted_feat)

########################################################################################################################
###########################################  EnhancedReprogramCrossAttn    #############################################
########################################################################################################################
# 交叉注意力模块，让轨迹特征主动查询地图特征
class EnhancedReprogramCrossAttn(nn.Module):
    def __init__(self, llm_dim=4096, n_heads=8):
        super().__init__()
        
        # 轨迹特征增强
        self.traj_enhance = nn.Sequential(
            nn.Linear(llm_dim, llm_dim),
            nn.GELU(),
            nn.LayerNorm(llm_dim))
        
        # 双向注意力机制
        self.traj2map_attn = nn.MultiheadAttention(llm_dim, n_heads, batch_first=True)
        self.map2traj_attn = nn.MultiheadAttention(llm_dim, n_heads, batch_first=True)
    ...
    def forward(self, traj_feat, map_feat):
        # 轨迹特征增强
        enhanced_traj = self.traj_enhance(traj_feat)
        
        # 轨迹→地图注意力
        map_aware, _ = self.traj2map_attn(
            query=map_feat,
            key=enhanced_traj,
            value=enhanced_traj)
        
        # 地图→轨迹注意力
        traj_aware, _ = self.map2traj_attn(
            query=enhanced_traj,
            key=map_feat.expand(-1, enhanced_traj.size(1), -1),
            value=map_feat.expand(-1, enhanced_traj.size(1), -1))
        
        # 门控融合
        gate = self.gate(torch.cat([enhanced_traj, traj_aware], dim=-1))
        return gate * map_aware + (1-gate) * enhanced_traj
class FusionStrategy(nn.Module):
    def __init__(self, llm_dim):
        super().__init__()
        self.cross_attn = EnhancedReprogramCrossAttn(llm_dim=llm_dim)
        self.linear = nn.Linear(llm_dim, llm_dim)
        self.norm = nn.LayerNorm(llm_dim)
        
    def forward(self, traj_feat, map_feat):
        # 先交叉注意力
        x = self.cross_attn(traj_feat, map_feat) 
        # 再线性变换和归一化
        x = self.linear(x)
        return self.norm(x)
###########################################  Model    #############################################
...
class Model(nn.Module):
    def __init__(self, configs, llm_model, tokenizer):
        ...
        if self.use_map:
            ...
            ################## 对map_encoder 进行初始化
            # 修改地图编码器初始化
            self.map_encoder = MultiScaleAttnMapEncoder(
                llm_dim=configs.llm_dim
            )
            
            # 增强特征融合
            self.fusion_strategy = FusionStrategy(configs.llm_dim)
    ...
    def forward(self, batch_agent_vec, batch_nbr_vec, batch_nbr_vec_mask, batch_nbr_rlpos, batch_nbr_rlpos_mask, batch_vehicle_map=None):
        ...
        if self.use_map:
            ...
            stage2 = self.fusion_strategy(reprogrammed_rep, map_encoded)

            llm_in = torch.cat([prompt_embeddings, stage2], dim=1)
        else:
