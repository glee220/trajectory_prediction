...
class EnhancedCrossModalAttention(nn.Module):
    def __init__(self, d_model=4096, n_heads=8, llm_dim=4096):
        ...
        # 增强后的投影层，使用多层感知机进行非线性变换
        self.traj_projection = nn.Sequential(
            nn.Linear(llm_dim, llm_dim),
            nn.LayerNorm(llm_dim),
            nn.GELU()  # 使用GELU激活函数提高表达能力
        )
        ...
        # 复杂的门控融合机制，增加了更多的非线性变换
        self.fusion_gate = nn.Sequential(
            nn.Linear(llm_dim * 2, llm_dim),
            nn.ReLU(),
            nn.LayerNorm(llm_dim),
            nn.Sigmoid()  # 门控使用Sigmoid，控制特征的融合比例
        )
...
class Model(nn.Module):
    def __init__(self, configs, llm_model, tokenizer):
        ...
        if self.use_map:
            ...
            # 使用增强后的MapEncoder
            self.map_encoder = EnhancedMapEncoder(map_channels=3, d_model=128)
            # 使用增强后的CrossModalAttention
            self.early_cross_attn = EnhancedCrossModalAttention(d_model=128)
    ...
    def forward(self, batch_agent_vec, batch_nbr_vec, batch_nbr_vec_mask, batch_nbr_rlpos, batch_nbr_rlpos_mask, batch_vehicle_map=None):
        ...
        if self.use_map:
            ...
            fused_features = self.feature_fusion(
                torch.cat([reprogrammed_rep, fused_lowdim], dim=-1)
            )
            # 如果使用地图，融合地图编码和轨迹数据
