...
class EnhancedMapEncoder(nn.Module):
    def __init__(self, map_channels=3, d_model=128):
        super().__init__()
...
class LowDimCrossAttention(nn.Module):
    def __init__(self, d_model=128, n_heads=8):
        ...
        self.map_proj = nn.Linear(d_model, d_model)
    ...
    def forward(self, traj_feat, map_feat):
        # traj_feat: [B, T, 128], map_feat: [B, 1, 128]
        traj_proj = self.traj_proj(traj_feat)
        map_proj = self.map_proj(map_feat)
        
        # 轨迹到地图的注意力
        attn_out, _ = self.attention(
            query=traj_proj,
            key=map_proj.expand(-1, traj_feat.size(1), -1),
            value=map_proj.expand(-1, traj_feat.size(1), -1)
        )
        
        # 门控融合
        gate_input = torch.cat([traj_feat, attn_out], dim=-1)
        gate = self.fusion_gate(gate_input)
        return gate * attn_out + (1 - gate) * traj_feat
###########################################  Model    #############################################
########################################################################################################################
class Model(nn.Module):
    def __init__(self, configs, llm_model, tokenizer):
        ...
        if self.use_map:
            ...
            ################## 对map_encoder 进行初始化
            # 修改地图编码器
            self.map_encoder = EnhancedMapEncoder(map_channels=3, d_model=128)  # 输出128维
            
            # 新增低维交叉注意力
            self.early_cross_attn = LowDimCrossAttention(d_model=128)
    ...
    def forward(self, batch_agent_vec, batch_nbr_vec, batch_nbr_vec_mask, batch_nbr_rlpos, batch_nbr_rlpos_mask, batch_vehicle_map=None):
        ...
        map_encoded = self.map_encoder(batch_vehicle_map).unsqueeze(1)  # [B, 1, 128]
        # 早期低维交叉注意力
        fused_lowdim = self.early_cross_attn(encoded_rep, map_encoded)  # [B, T, 128]
        #################################################### 学习prototype word
        ...
        if self.use_map:                       
            ...
            reprogrammed_rep = self.reprogramming_layer(fused_lowdim, source_embeddings, source_embeddings)
