...
def main():
    ...
    with torch.no_grad():
        for batch in test_dl:
            ...
            agent_pos = batch_agent_pos.to(device, dtype=precision)
            ...
            with caster:
                ...
                # 用于保存数据的部分
                for i in range(x_agent.shape[0]):  # 对每个batch中的每个样本进行保存
                    # 获取周车数据
                    nbr_vec = batch_nbr_vec[i]  # 周车向量数据
                    nbr_rlpos = batch_nbr_rlpos[i]  # 周车相对位置数据
                    nbr_vec_mask = batch_nbr_vec_mask[i]  # 周车向量掩码
                    nbr_rlpos_mask = batch_nbr_rlpos_mask[i]  # 周车相对位置掩码
                    sample_data = {
                        "historical_trajectory": x_agent[i].to(torch.float32).cpu().numpy(),  # 转换为float32
                        "ground_truth_trajectory": y_pos[i].to(torch.float32).cpu().numpy(),  # 转换为float32
                        "predicted_trajectory": preds[i].to(torch.float32).cpu().numpy(),     # 转换为float32
                        "map_data": vehicle_map[i].to(torch.float32).cpu().numpy(),             # 转换为float32
                        "agent_pos": agent_pos[i].to(torch.float32).cpu().numpy(),             # 转换为float32
                        # 添加周车相关信息
                        "neighbor_vectors": nbr_vec.to(torch.float32).cpu().numpy(),
                        "neighbor_relative_positions": nbr_rlpos.to(torch.float32).cpu().numpy(),
                        "neighbor_vector_mask": nbr_vec_mask.cpu().numpy(),
                        "neighbor_rlpos_mask": nbr_rlpos_mask.cpu().numpy()
                    }
